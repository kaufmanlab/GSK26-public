...
%                 color    default = 'k'
%              fontSize    default = 8
%       Hank:  fontName    default = 'Helvetica Neue'
% 
...
% normal or inverted axis ticks (inverted = top for horizontal, rhs for vertical)
Pfields{ipf} = 'invertTicks'; ipf=ipf+1;
if ~isempty(varargin) && isfield(varargin{1}, 'invertTicks')
    invertTicks = varargin{1}.invertTicks; % just keep the first letter ('h' for horizontal, 'v' for vertical)
else
    invertTicks = 0; % default is normal axis
end
...
if invertTicks == 1, tickLength = -tickLength; end  % make negative if axis is inverted
...
if ~isempty(varargin) && isfield(varargin{1}, 'fontName')
    ...
    fontName = 'Helvetica Neue'; % default is Helvetica Neue my fav font
end
...
for i = 1:length(tickLabelLocations)
    ...
    set(h,'HorizontalA', NalignH, 'VerticalA', NalignV, 'fontsize', fontSize, 'color', color, 'FontName', fontName);
end
...
set(h,'HorizontalA', LalignH, 'VerticalA', LalignV, 'fontsize', fontSize, 'color', color, 'FontName', fontName);
if axisOrientation == 'v', set(h,'rotation',90); end
if invert, set(h, 'rotation', -90); end
% DONE PLOTTING
