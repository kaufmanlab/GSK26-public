...
% data = multiEventLockKinematics(R, eventName, tPts, ...)
%
% Repair, smooth, and time-lock video tracking data from DeepLabCut (3D
...
% data = multiEventLockKinematics(R, 'grabTime', -200:10:300);
%
%
% INPUTS
% R         -- from mergeDLC3R
% M         -- from mergeDLC3R
% eventNames -- the fields in R to lock to, e.g., 'grabTime'
...
ip.addParameter('addCentOffsetXYZ', 1);
ip.parse(varargin{:});
...
for e = 1:length(eventNames)
    ...
    if ~isfield(R, eventName)
        ...
        if ~isfield(R, eventName)
            error('multiEventLockKinematics:badEventName', 'all eventNames must be names of fields in R');
        end
    end
end

%% For computing skeleton links and dist2Spout
skeletonInds = sub2ind([length(M.markerNames), length(M.markerNames)], M.skeleton(:,1), M.skeleton(:,2));
...
for tr = 1:length(R)
    ...
    data(tr).success = R(tr).success;
    data(tr).allGrabTimes = R(tr).allGrabTimes;
    data(tr).syncTime = R(tr).syncTime;
    ...
    if any(cellfun(@(x) isnan(R(tr).(x)), eventNames))
    ...
    if isempty(R(tr).posXYZ)
        % No posXYZ data for this trial, try uncleaned
        if isempty(R(tr).posXYZAll)
    ...
    % Set 0 at first event time
    camTimes = R(tr).cam(1).times + R(tr).syncTime - R(tr).(eventNames{1});
    % Get time between two events
    interval = R(tr).(eventNames{2}) - R(tr).(eventNames{1});
    % If asked for equal times before and after second event, double
    % interval, this will round down last time bin, ie interval*2=1039 will
    % give preT:10:1030, max discrepancy is tBase - 1 ms for usual tBase=10
    if strcmp(postT, 'equal')
        tPts = preT:tBase:(interval * 2);
    else
        tPts = preT:tBase:(interval + postT);
    end
    
    % If we want multiple lags, we will lock enough data to pull out all
    % desired lags
    if length(lags) > 1
        % Create a struct of tPts shifted by each desired lag
        % must be done trial by trial as they have different lengths
        tPtsLagged = zeros(length(lags), length(tPts));
        for lag = 1:length(lags)
            tPtsLagged(lag,:) = tPts + lags(lag);
        end
        % Get unique values across all lags
        tPtsNoLag = tPts;
        tPts = unique(tPtsLagged(:));
    else
        % Just do one lag
        tPts = tPts + lags;
    end
    ...
    data(tr).tPts = tPts;
    data(tr).eventInterval = interval;
    data(tr).preEvent1T = preT;
    data(tr).tBase = tBase;
    ...
    if ip.Results.addCentOffsetXYZ
        data(tr).CentOffsetXYZ = data(tr).CposXYZ - data(tr).posXYZ(:, :, 1:13);
    end
    
    %% Add distance to spout, and tongue
    
    if computeDist2Spout
    [data(tr).dist2Spout, data(tr).dist2Tongue] = getDists(data(tr), spoutPoseL, spoutPoseR);
    end

    %% Add lengths between skeleton links
    
    data(tr).linkLengths = getLinkLengths(data(tr), skeletonInds);
    ...
    %% Add lags, just doing for aperture, angles and velocities atm
    % Cut out relevant portion of each trial for the different lags if
    % necessary
    
    if length(lags) > 1
        % Crop out each lag
        for lag = 1:length(lags)
            idx = ismember(data(tr).tPts, tPtsLagged(lag,:));
            data(tr).(sprintf('lag%ijointAngles',lag)) = data(tr).jointAngles(idx,:);
            data(tr).(sprintf('lag%ijointVelocities',lag)) = data(tr).jointVelocities(idx,:);
            data(tr).(sprintf('lag%iaperture',lag)) = data(tr).aperture(idx);
            
            % Check validity
            if any(isnan(data(tr).(sprintf('lag%ijointAngles',lag))))
                warning('Trial %i at lag %i has missing data', tr, lags(lag))
            end
            
        end
        % Keep timing info
        data(tr).lagInfo.tPts = tPtsLagged;
        data(tr).lagInfo.lags = lags;
    end
...
function [dist2Spouts, dist2Tongue] = getDists(data, spoutPoseL, spoutPoseR)
...
if data.locked
    ...
    for t = 1:tBins
        ...
        % Get vector pointing from current position to target spout
        onTargetVec = squeeze(posXYZ(t,:,end))' - squeeze(posXYZ(t,:,:));
        % Save the norm
...
function linkLengths = getLinkLengths(data, skeletonInds)

if data.locked
    posXYZ = data.posXYZ;
    linkLengths = zeros(length(skeletonInds), size(posXYZ,1));
    for tPt = 1:size(posXYZ,1)
        posXYZt = squeeze(posXYZ(tPt,:,:));
        distsFrame = eucDists(posXYZt);
        linkLengths(:,tPt) = distsFrame(skeletonInds);
    end
else
    linkLengths = [];
end
